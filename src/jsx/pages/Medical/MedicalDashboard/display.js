...
import { saveApplicantDetails } from '../../../../store/actions/Medical/action'
    ...
    <>
            ...
            {users.length > 0 &&
              users.map(item => (
                <tr role='row' className='odd' key={item._id}>
                  ...
                  <td>{item.userId.lgaOfOrigin || 'n/a'}</td>
                  <td>{item.userId.lgaOfResident || 'n/a'}</td>
                  <td className=''>
                    <CustomButton
                      style={{
                        background: `${
                          item.status === 'passed' ? 'green' : 'transparent'
                        }`,
                        border: `${
                          item.status === 'passed'
                            ? 'none'
                        ...
                        color: `${
                          item.status === 'passed' ? 'white' : '#02024D'
                        }`
                      }}
                      onClick={() => {
                        httpMoveApplicant(item.userId._id);
                      }}
                    >{`${
                      item.status === 'passed' ? 'Verified' : 'Move to next'
                    }`}</CustomButton>
                    {/* <Link
                      to={
                        routes.viewApplicantProfile.split(':')[0] +
                        item.userId._id
                      }
                    >
                      <CustomButton
                        onClick={() => {
                          displayApplicantInfo(item)
                        }}
                      >
                        View
                      </CustomButton>
                    ...
                    <Link
                      to={
                        routes.medicalScreening.split(':')[0] +
                        jobId +
                        '/' +
                        item.userId._id
                      }
      ...
      >
        <div className='card p-5' style={{ borderRadius: '0.44rem' }}>
          <h4>
            {user.fullName} {user.otherName} {user.surname}
          </h4>
          <hr></hr>
          ...
          <div>
            <p className='mb-2'>
              {' '}
              <EnvelopeSimple size={20} style={{ marginRight: '1rem' }} />{' '}
              Email: <strong>{user.email} </strong>
            </p>
            {user.userId && (
              <p className='mb-2'>
                {' '}
                <Phone size={20} style={{ marginRight: '1rem' }} /> Phone No:{' '}
                <strong>{user.userId.phone}</strong>
              </p>
            )}
            {user.gender && (
              <p className='mb-2'>
            ...
            {user.DOB && (
              <p className='mb-2'>
                {' '}
                <Calendar size={20} style={{ marginRight: '1rem' }} /> Date of
                birth: <strong>{Date(user.DOB).split('G')[0]}</strong>
              </p>
            )}
            {user.maritalStatus && (
              <p className='mb-2'>
                {' '}
                <Users size={20} style={{ marginRight: '1rem' }} /> Marital
                status: <strong>{user.maritalStatus}</strong>
              </p>
            )}
            {user.attitudeTestState && (
              <p className='mb-2'>
                {' '}
                <Buildings size={20} style={{ marginRight: '1rem' }} /> Aptitude
                test state: <strong>{user.attitudeTestState}</strong>
              </p>
            )}
            {user.educationalQualification && (
              <ul className='mb-2'>
                {' '}
                <GraduationCap size={20} style={{ marginRight: '1rem' }} />{' '}
                Educational qualification:
                {user.educationalQualification.length > 0 &&
                  user.educationalQualification.map(item => (
                    <strong>
                      <li key={item._id} className='mx-5'>
                        - {item.certificateObtained}
                      </li>
                    </strong>
                  ))}
              </ul>
            ...
            {user.languageSpoken && (
              <ul className='mb-2'>
                {' '}
                <Translate size={20} style={{ marginRight: '1rem' }} />{' '}
                Language:
                {user.languageSpoken.length > 0 &&
                  user.languageSpoken.map(item => (
                    <strong>
            ...
            {user.hobbies && (
              <p className='mb-2'>
                ...
                <PersonSimpleRun
                  ...
                  style={{ marginRight: '1rem' }}
                />{' '}
                Hobbies: <strong>{user.hobbies}</strong>
              </p>
            )}
            {user.userId && (
              <p className='mb-2'>
                ...
                <GlobeHemisphereEast
                  ...
                  style={{ marginRight: '1rem' }}
                />{' '}
                State of origin: <strong>{user.userId.stateOfOrigin}</strong>
              </p>
            )}
            {user.userId && (
              <p className='mb-2'>
                ...
                <GlobeHemisphereEast
                  ...
                  style={{ marginRight: '1rem' }}
                />{' '}
                LGA of origin: <strong>{user.userId.lgaOfOrigin}</strong>
              </p>
            )}
            {user.userId && (
              <p className='mb-2'>
                {' '}
                <House size={20} style={{ marginRight: '1rem' }} /> State of
                residence: <strong>{user.userId.stateOfResident}</strong>
              </p>
            )}
            {user.userId && (
              <p className='mb-2'>
                ...
                <GlobeHemisphereEast
                  ...
                  style={{ marginRight: '1rem' }}
                />{' '}
                LGA of residence: <strong>{user.userId.lgaOfResident}</strong>
              </p>
            )}
            {user.religion && (
              <p className='mb-2'>
                {' '}
                <ShieldPlus size={20} style={{ marginRight: '1rem' }} />{' '}
            ...
            {user.village && (
              <p className='mb-2'>
                ...
                <GlobeHemisphereEast
                  ...
                  style={{ marginRight: '1rem' }}
                />{' '}
                Village: <strong>{user.village}</strong>
              </p>
            )}
...
const mapDispatchToProps = dispatch => ({
  ...
  saveApplicantDetails: data => dispatch(saveApplicantDetails(data))
})
