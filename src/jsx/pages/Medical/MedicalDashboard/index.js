  ...
  async function getAllApplicants () {
    try {
      const response = await axios(
        `https://employer-center-api.herokuapp.com/api/v1/pipeline/fetch-pipeline-candidate/${id}`,
        {
          headers: {
            authorization: `Bearer ${auth.token}`,
            'Content-type': 'application/json'
      ...
      >
        <div className='d-flex justify-content-between mb-4'>
          ...
          <div className='d-flex'>
            <Link
              to={`${
                routes.formBuilder.split(':')[0]
              }${organization_id}/${jobId}/${id}`}
            >
              <CustomButton>
                {' '}
                <FirstAidKit size={22} /> Manage Medical Form
              </CustomButton>
            </Link>
            <CSVLink
          ...
          >
            <div className=''>
              ...
              <h4 className='mt-2' style={{ color: '#fff' }}>
                {sortApplicantByStatus([...filterSkills([...searchByNameOrEmail().filter(item => item.userId !== null)])]).length}
              </h4>
            </div>
          </div>
