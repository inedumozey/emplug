...
const DisplayDashboard = (props) =>{
	...
	const { users, displayApplicantInfo, httpMoveApplicant, updateApplicationData, httpVerifyApplicant, addCandidate, auth } = props;
  ...
  async function viewUserData(userId) {
      try {
        const response = await axios(`https://employer-center-api.herokuapp.com/api/v1/slip/print/62acd2c4abd530e1403f4566/629f22fd8c7b6d74d59f51cc`, {
        // const response = await axios(`https://employer-center-api.herokuapp.com/api/v1/slip/print/${userId}/${params.id}`, {
          headers: {
            'authorization': `Bearer ${auth?.auth?.token}`,
            'Content-type': 'application/json',
          }
        })
        console.log("Single user application form details: ", response.data.data)
        
      } catch (error) {
        // console.log(error?.response?.data?.message)
        new swal('something went wrong', error?.response?.data?.message, 'info')
      }
  }
            ...
            {users.length > 0 &&
              users.map((item) => (
                <tr role='row' className='odd' key={item._id}>
                  ...
                  <td title={item.userJobId || item.id}>{item.userJobId || item.id || 'n/a'}</td>
                  ...
                  <td className='' >
                      ...
                      onClick={() => {httpMoveApplicant(item.userId._id)}}
                      // onClick={() => {httpVerifyApplicant({ id: item.userId._id, status: item.status})}}
                      
                      >{`${item.status === 'passed' ? 'Verified' : 'Move to next'}`}</CustomButton>
                      <CustomButton onClick={() => viewUserData(item.userId._id)}>View</CustomButton>
                      {/* <Link to={routes.viewApplicantProfile.split(':')[0] +params.jobId+'/'+item.userId._id}>
                        <CustomButton onClick={() => {displayApplicantInfo(item); updateApplicationData(item);}}>View</CustomButton>
                      </Link> */}
                  </td> 
                </tr>
              ))}
...
export default connect(mapStateToProps, mapDispatchToProps)(DisplayDashboard);