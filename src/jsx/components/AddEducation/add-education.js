...
import { addNewEducation, fieldToEdit, httpUpdateProfessionalProfile, getProProfile } from '../../../store/actions/MyPortfolio/my-portfolio.actions';





function AddEducation({ auth, fieldToEdit, httpUpdateProfessionalProfile, myPortfolio, setShow, getProProfile }) {
	...
	function checkIfInstitutionExist() {
		...
		return myPortfolio.educationalQualifications.map(item => item._id ).includes(info._id);
	}
	...
	function updateInstitution() {
		...
		if (checkIfInstitutionExist()) {
			const selectInstitution = myPortfolio.educationalQualifications.find(item => item._id === myPortfolio.edit._id);
			...
			const data = myPortfolio.educationalQualifications.filter(item => item._id !== myPortfolio.edit._id);
		...
		const newData = {
			...myPortfolio,
			educationalQualifications: [...myPortfolio.educationalQualifications, newPro]
		}
		httpUpdateProfessionalProfile(newData, auth.user._id, auth.token);
		// getProProfile(newData.user, auth.token);
	...
	return (
		...
		<form style={{padding: '2rem'}} onSubmit={updateEducation}>
				<div style={{ width: '100%', marginBottom: '2rem', borderBottom: '1px solid #eee', paddingBottom: '1rem'}}>
					<h3 style={{color: '#7f7f7f'}}>Add Education</h3>
				</div>
